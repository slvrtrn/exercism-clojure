...
(defn learning-list
  ...
  []
  (let [lst (new-list)
        with-clojure (add-language lst "Clojure")
        with-lisp (add-language with-clojure "Lisp")
        without-lisp (remove-language with-lisp)
        with-java (add-language without-lisp "Java")
        with-js (add-language with-java "JavaScript")]
    (count-languages with-js)))
